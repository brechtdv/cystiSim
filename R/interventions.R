...
function(x, coverage, efficacy.oxf, efficacy.vac,
  ...
  is_treated <- rbinom(sum(id), 1, coverage) == 1
  is_vaccinated <- is_treated

  ## randomize effective OXF treatment over covered pigs
  is_treated[is_treated == 1] <-
    rbinom(sum(id[is_treated]), 1, efficacy.oxf) == 1

  ## randomize effective VAC treatment over covered pigs
  is_vaccinated[is_vaccinated == 1] <-
    rbinom(sum(id[is_vaccinated]), 1, efficacy.vac) == 1
  ...
  is_immune <-
    ...
    (x$pig$cysti[id][is_treated] == 1 |
     x$pig$cysti_immature[id][is_treated] == 1)
  x$pig$immunity[id][is_treated][is_immune] <- immunity.oxf
  ...
  is_immune <- !is.na(x$pig$time_since_vaccination[id][is_vaccinated]) &
               x$pig$time_since_vaccination[id][is_vaccinated] > 0
  x$pig$immunity[id][is_vaccinated][is_immune] <- immunity.vac
